...
class ActorCriticAgent(PolicyGradientAgent):
    ...
    """
    def __init__(self, env:IdsGameEnv, config: PolicyGradientAgentConfig, hp_to_tune):
        """
    ...
    def train(self) -> ExperimentResult:
        ...
        if self.config.save_dir is not None:
            if self.hp_to_tune == "epsilon_decay_":
                ...
                x = str(time.time())
            self.train_result.to_csv(self.config.save_dir + "/ac_train_results_" + self.hp_to_tune + x + ".csv")
            self.eval_result.to_csv(self.config.save_dir + "/ac_eval_results_" + self.hp_to_tune + x + ".csv")
